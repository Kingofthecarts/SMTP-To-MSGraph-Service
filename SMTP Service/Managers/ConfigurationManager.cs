    ...
    public class ConfigurationManager
    {
        ...
        public AppConfig LoadConfiguration()
        {
            ...
            try
            {
                ...
                var json = ReadJsonFile(_configPath);
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine("Recreating configuration with defaults...");
        ...
        public void SaveConfiguration(AppConfig config)
        {
            try
            {
                ...
                WriteJsonFile(_configPath, json);
        ...
        public SmtpConfiguration LoadSmtpConfiguration()
        {
            ...
            try
            {
                var json = ReadJsonFile(_smtpPath);
                var config = JsonSerializer.Deserialize<SmtpConfiguration>(json, JsonOptions);
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine("Recreating SMTP configuration with defaults...");
                
                BackupCorruptedFile(_smtpPath);
            }
        ...
        public UserConfiguration LoadUserConfiguration()
        {
            ...
            try
            {
                var json = ReadJsonFile(_userPath);
                var config = JsonSerializer.Deserialize<UserConfiguration>(json, JsonOptions);
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine("Recreating user configuration with defaults...");
                
                BackupCorruptedFile(_userPath);
            }
        ...
        public GraphConfiguration LoadGraphConfiguration()
        {
            ...
            try
            {
                var json = ReadJsonFile(_graphPath);
                var config = JsonSerializer.Deserialize<GraphConfiguration>(json, JsonOptions);
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine("Recreating Graph configuration with defaults...");
                
                BackupCorruptedFile(_graphPath);
            }
        ...
        /// <summary>
        /// Write JSON file with UTF-8 encoding (no BOM) and atomic write
        /// </summary>
        ...
        /// <summary>
        /// Backup a corrupted file before recreating
        /// </summary>
        private void BackupCorruptedFile(string path)
        {
            try
            {
                if (File.Exists(path))
                {
                    ...
                    var corruptedPath = $"{path}.corrupted_{timestamp}";
                    File.Copy(path, corruptedPath, true);
                    Console.WriteLine($"Corrupted file backed up to: {Path.GetFileName(corruptedPath)}");
                    Log.Warning($"Corrupted configuration file backed up: {corruptedPath}");
                }
            }
            catch (Exception ex)
            {
                Console.WriteLine($"WARNING: Could not backup corrupted file: {ex.Message}");
            }
        }
        #endregion
        ...
        {
            var json = JsonSerializer.Serialize(config, JsonOptions);
            return JsonSerializer.Deserialize<UserConfiguration>(json, JsonOptions) ?? new UserConfiguration();
        }

        private GraphConfiguration CloneGraphConfig(GraphConfiguration config)
        {
            var json = JsonSerializer.Serialize(config, JsonOptions);
            return JsonSerializer.Deserialize<GraphConfiguration>(json, JsonOptions) ?? new GraphConfiguration();
        }
        #endregion
    }
}
